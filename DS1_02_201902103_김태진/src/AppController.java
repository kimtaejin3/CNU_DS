...
public class AppController {
	...
	private void addCoin () {
		if(this.coinBag().isFull()) {
			AppView.outputLine("- 동전 가방이 꽉 차서 넣을 수 없습니다. ");
		}
		else
		{
			...
			while(coinValue<0) 
			{
				AppView.output("음수를 입력하지 마세요.\n? 동전 가방의 크기, 즉 가방에 들어갈 동전의 최대 개수를 입력하시오:"); 
	...
	public void run() {
		...
		//음수를 입력하면 논리상 맞지 않음.
		while(coinBagSize<0) {AppView.output("음수를 입력하지 마세요.\n? 동전 가방의 크기, 즉 가방에 들어갈 동전의 최대 개수를 입력하시오:"); coinBagSize=AppView.inputCapacityOfCoinBag(); }
